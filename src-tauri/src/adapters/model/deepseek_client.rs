...
struct DeepSeekToolCallDelta {
    ...
    id: Option<String>,
    #[serde(default)]
    function: Option<DeepSeekFunctionCallDelta>,
}

#[derive(Deserialize)]
struct DeepSeekFunctionCallDelta {
    #[serde(default)]
    name: Option<String>,
    #[serde(default)]
...
fn convert_message(msg: &Message) -> DeepSeekMessage {
    ...
    let (content, tool_calls, tool_call_id) = if msg.role == MessageRole::Assistant {
        ...
        for part in &msg.content {
            match part {
                ContentPart::ToolUse { id, name, input } => {
                    tool_calls_vec.push(DeepSeekToolCall {
                        id: id.clone(),
        ...
        let content = if text_parts.is_empty() {
            DeepSeekContent::Text(String::new())
        } else {
            DeepSeekContent::Text(text_parts.join("\n"))
        };
        ...
        if let Some(ContentPart::ToolResult { tool_use_id, content, .. }) = msg.content.first() {
            (DeepSeekContent::Text(content.clone()), None, Some(tool_use_id.clone()))
        } else {
            (DeepSeekContent::Text(String::new()), None, None)
        }
        ...
        let parts = msg.content.iter().map(|p| match p {
            ...
            _ => DeepSeekContentPart::Text { text: String::new() },
        }).collect();
        (DeepSeekContent::Parts(parts), None, None)
    } else {
        let text = msg.content.iter()
            .filter_map(|p| if let ContentPart::Text { text } = p { Some(text.as_str()) } else { None })
            .collect::<Vec<_>>()
            .join("\n");
        (DeepSeekContent::Text(text), None, None)
    };

    let mut result = DeepSeekMessage { role: role.to_string(), content, tool_calls: None, tool_call_id: None };
...
impl ModelClient for DeepSeekClient {
    async fn stream_completion(
        ...
        let messages: Vec<DeepSeekMessage> = request.messages.iter().map(convert_message).collect();

        let tools = if request.tools.is_empty() {
            None
        } else {
            Some(request.tools.iter().map(|t| DeepSeekTool {
                tool_type: "function".to_string(),
                function: DeepSeekFunction {
                    name: t.name.clone(),
        ...
        let chunk_stream = stream.map(move |chunk_result| {
            ...
            for line in text.lines() {
                ...
                let parsed: DeepSeekStreamResponse = serde_json::from_str(data)
                    .map_err(|e| ModelError::InvalidResponse(e.to_string()))?;

                if let Some(usage) = parsed.usage {
                    let cache_read = usage.prompt_cache_hit_tokens.unwrap_or(0);
                    let cache_creation = usage.prompt_cache_miss_tokens.unwrap_or(0);
